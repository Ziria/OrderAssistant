...
//    }
    ...
    //[showDateBtn addTarget:self action:@selector(showDateAction) forControlEvents:UIControlEventTouchUpInside];
    
    //[showTimeBtn addTarget:self action:@selector(showTimeAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)cancelAppointment
{
    if (delegate == nil) {
        ...
        [delegate dismissAddappointmentViewController:self];
    }
}
- (void)submitAppointmentAndDish{
    ...
    if ( [@"" isEqual:firstName.text]||[@"" isEqual:appCustomerCount.text]||[@"" isEqual:appTableCount.text])
    {
        ...
        dishJSon= [[NSString alloc] initWithData:dishJsonData encoding:NSUTF8StringEncoding];
